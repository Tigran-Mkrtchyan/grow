...
export const VersionProvider = ({ children }: TVersionContextProps) => {
  const [versionLastCheckDate, setVersionLastCheckDate] = useState(
    localStorage.getItem('versionLastCheckDate') || '',
  );
  const [newVersionAvailable, setNewVersionAvailable] = useState(false);
  const versionModalRef = useRef({
    open: (waiting: ServiceWorker | null) => {
      console.log(waiting);
    },
  });
  ...
  useEffect(() => {
    serviceWorker.register({
      /* onInit: onSWRegistered, */ onUpdate: onSWUpdate,
    });
